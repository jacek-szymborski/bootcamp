...
class Basket:
    ...
    def generate_report(self): #bez f stringa funkcja format działa tak, że po kolei wstwia w nawiasy klamrowe
...
W sumie: {}
        ...
        for entry in self.entries:
            temp += f"- {entry.product.name}{[entry.product.ID]}, cena: {self.product.price} x {self.quantity} "

        return output.format(total_price)
